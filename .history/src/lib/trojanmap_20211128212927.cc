...
std::vector<std::string> TrojanMap::CalculateShortestPath_Bellman_Ford(
  ...
  distance[source] = 0;
  p[source].push_back(source);
  for(int i=0;i<data.size()-1;i++){
    for(auto it:data){
      for (auto n:it.second.neighbors){
        double a=CalculateDistance(it.first,n);
        if(distance[it.first]>a+distance[n]){
          distance[it.first]=a+distance[n];
          p[it.first]=p[n];
          p[it.first].push_back(it.first);
        }
      }
    }
  }
