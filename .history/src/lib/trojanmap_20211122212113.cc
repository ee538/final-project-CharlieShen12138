...
void cyclehelper(std::map<std::string, Node>& data,std::string currentID,std::unordered_map<std::string, int>& visited_points,std::unordered_map<std::string, std::string>& prev_points,std::vector<std::string>& cycle){
  ...
  for(auto next:data){
    ...
    if(visited_points.find(next) == visited_points.end()){         //this point is outside the squre
      continue;
    }
    if(visited_points[currentID]==0){
      prev_points[next] = currentID;       //prepare for next loop
      cyclehelper(data, next, visited_points, prev_points, cycle);   //run next loop
    }
    else if(visited_points[next]==1 && next != prev_points[currentID]){       //it's not a line but a loop
      std::string it = currentID;
      while(it != next){                     //create the cycle path vector
        cycle.push_back(it);
        it = prev_points[it];
      }
      cycle.push_back[it];
