...
std::pair<double, std::vector<std::vector<std::string>>> TravellingTrojan_2opt(
  ...
  int n = crrentsol.size();        //loop length
  ...
  while(app<=15){
    next_loop: double currentdis = CalculatePathLength(currentsol);    //calculate current length 
     for(int i=1; i<n-2; i++){           //first point
       ...
       double newdist = CalculatePathLength(current_method);               //calculate new distance
 ...
 */
bool TrojanMap::CycleDetection(std::vector<double> &square) {                          //dis 11
  return false;
}
