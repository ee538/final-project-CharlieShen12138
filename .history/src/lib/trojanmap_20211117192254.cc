...
std::string fminNv(std::map<std::string, double> distance, std::map<std::string, int> visitedpoint){
  ...
  for(auto temp:distance){
    if((visitedpoint[temp.first]==0) && (distance[temp.first]<=min)){
      min = distance[temp.first];
      nextmin = temp.first;
    }
  }
  return nextmin;
...
std::vector<std::string> TrojanMap::CalculateShortestPath_Dijkstra(
    ...
    std::string sourse;
    std::string end;
    std::map<std::string, int> visitedpoint;
    std::map<std::string, std::string> previouspoint;
    ...
    for(auto cweight:data){                                       //initialize maps for the dijksatra algorithm
      ...
      sourse = cweight.first;}                                    //start point
      if(cweight.second.name.compare(location2_name)==0){
      end = cweight.first;}                                       //destination
    }
